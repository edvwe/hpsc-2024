...
// Defined matrix struct since vector struct can't directly be sent to gpu memory
struct matrix
{
    int rows;
    int cols;
          ...
          const int ny)
{
    ...
    b->elems[i] = rho *
                  (1.0 / dt * ((u->elems[i + 1] - u->elems[i - 1]) / (2.0 * dx) + (v->elems[i + nx] - v->elems[i - nx]) / (2.0 * dy)) -
                   ((u->elems[i + 1] - u->elems[i - 1]) / (2.0 * dx)) * ((u->elems[i + 1] - u->elems[i - 1]) / (2.0 * dx)) -
                   2.0 * ((u->elems[i + nx] - u->elems[i - nx]) / (2.0 * dy) * (v->elems[i + 1] - v->elems[i - 1]) / (2.0 * dx)) -
                   ((v->elems[i + nx] - v->elems[i - nx]) / (2.0 * dy)) * ((v->elems[i + nx] - v->elems[i - nx]) / (2.0 * dy)));
}

// ------------------- Functions for computing p -------------------

__global__ void
copy_pn(const matrix *p,
        matrix *pn,
        const int nx,
        const int ny)
{
...
__global__ void calc_u_v(matrix *u,
                         matrix *v,
                         const matrix *un,
                         const matrix *vn,
                         const matrix *p,
                         ...
                         const int ny)
{
    ...
    u->elems[i] = un->elems[i] - un->elems[i] * dt / dx * (un->elems[i] - un->elems[i - 1]) -
                  un->elems[i] * dt / dy * (un->elems[i] - un->elems[i - nx]) -
                  dt / (2 * rho * dx) * (p->elems[i + 1] - p->elems[i - 1]) +
                  nu * dt / (dx * dx) * (un->elems[i + 1] - 2 * un->elems[i] + un->elems[i - 1]) +
                  nu * dt / (dy * dy) * (un->elems[i + nx] - 2 * un->elems[i] + un->elems[i - nx]);
    v->elems[i] = vn->elems[i] - vn->elems[i] * dt / dx * (vn->elems[i] - vn->elems[i - 1]) -
                  vn->elems[i] * dt / dy * (vn->elems[i] - vn->elems[i - nx]) -
                  dt / (2 * rho * dx) * (p->elems[i + nx] - p->elems[i - nx]) +
                  nu * dt / (dx * dx) * (vn->elems[i + 1] - 2 * vn->elems[i] + vn->elems[i - 1]) +
                  nu * dt / (dy * dy) * (vn->elems[i + nx] - 2 * vn->elems[i] + vn->elems[i - nx]);
}
...
pad_u_v(matrix *u,
        ...
        const int ny)
{
    for (int y = 0; y < ny; y += nx)
    {
        // pad left and right most
        u->elems[y] = 0;          // leftmost col
        u->elems[y + nx - 1] = 0; // rightmost col
        v->elems[y] = 0;          // leftmost col
        v->elems[y + nx - 1] = 0; // rightmost col
    }
    for (int x = 0; x < nx; x++)
    {
        // pad top and bottom
            ...
            const int THREADS_PER_BLOCK)
{
    ...
    calc_u_v<<<BLOCKS, THREADS_PER_BLOCK>>>(u, v, un, vn, p, dx, dy, dt, rho, nu, nx, ny);
...
void init_matrix(matrix *&mat, const int nx, const int ny)
{
    ...
    // Allocate for elemtents
    cudaMallocManaged(&(mat->elems), mat->rows * mat->cols * sizeof(float));
...
int main()
{
    ...
    constexpr int THREADS_PER_BLOCK = 256; // maybe this could be higher? 512? 1024? try for best value
    ...
    for (int n = 0; n < nt; n++)
    {
        ...
        compute_u_v(u, v, un, vn, p, dt, dx, dy, rho, nu, nx, ny, BLOCKS, THREADS_PER_BLOCK);
